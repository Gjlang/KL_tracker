            ...
            @foreach ($billboard->images as $img)
                ...
                @if (file_exists($path))
                    <img src="{{ $path }}" alt="Billboard Image"
                        style="width: 48%; height: 370px; object-fit: contain; border: 1px solid #ccc; vertical-align: top;">
                @endif
            @endforeach
        </div>
        ...
        <table class="footer">
            <tr>
                <td class="remarks-col">
                    <strong>REMARK: The site is subject to availability, authority approval, and safety regulations. In
                        the event that the proposed sites are unavailable on the installation day - whether due to
                        changes in local council regulations,
                        site upgrades to a protocol road, the presence of existing boards from other parties, or
                        safety-related issues - Bluedale will proceed to install the board at a nearby location as close
                        as possible to the original site, or
                        suggest an alternative. Photo evidence will be provided as proof of installation. Replacement of
                        missing boards is only applicable at no extra charge for clients who purchase the current
                        promotion with a minimum 3-month contract or longer.

<aside
    ...
    <div class="h-14 flex items-center gap-3 px-4 border-b border-neutral-200">
        ...
        <div class="w-9 h-9 rounded-xl flex items-center justify-center cursor-pointer relative z-50"
            style="background:#22255b;" title="Close sidebar" @click.stop="close()">
    ...
    <nav class="flex-1 overflow-y-auto py-3" x-data="sidebarState()">
        ...
        {{-- Main Dashboard --}}
        <div class="px-3 mb-2">
            <a href="{{ route('dashboard') }}" @click="if (window.matchMedia('(max-width: 767px)').matches) close()"
        ...
        {{-- KLTG --}}
        <div class="px-3">
            <button @click="toggle('kltg')"
                class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-neutral-50">
                <span class="font-medium">KLTG</span>
                <svg class="w-4 h-4 transition-transform duration-200" :class="open.kltg ? 'rotate-180' : ''"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path stroke-linecap="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            <div x-show="open.kltg" x-transition:enter="transition ease-out duration-200"
        ...
        {{-- Social Media --}}
        <div class="px-3 mt-2">
            <button @click="toggle('media')"
                class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-neutral-50">
                <span class="font-medium">Social Media</span>
                <svg class="w-4 h-4 transition-transform duration-200" :class="open.media ? 'rotate-180' : ''"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path stroke-linecap="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            <div x-show="open.media" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 transform scale-95"
                x-transition:enter-end="opacity-100 transform scale-100"
                x-transition:leave="transition ease-in duration-150"
                x-transition:leave-start="opacity-100 transform scale-100"
        ...
        {{-- Outdoor --}}
        <div class="px-3 mt-2">
            <button @click="toggle('outdoor')"
                class="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-neutral-50">
                <span class="font-medium">Outdoor</span>
                <svg class="w-4 h-4 transition-transform duration-200" :class="open.outdoor ? 'rotate-180' : ''"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path stroke-linecap="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            <div x-show="open.outdoor" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 transform scale-95"
                x-transition:enter-end="opacity-100 transform scale-100"
                x-transition:leave="transition ease-in duration-150"
                x-transition:leave-start="opacity-100 transform scale-100"
                ...
                <a href="{{ route('billboard.index') }}"
                    @click="if (window.matchMedia('(max-width: 767px)').matches) close()"
                    class="block mt-1 mx-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('billboard.index') ? 'bg-[#22255b] text-white' : 'hover:bg-neutral-50' }}">
                    Outdoor Stock inventory
                </a>
                <a href="{{ route('billboard.availability.index') }}"
                    @click="if (window.matchMedia('(max-width: 767px)').matches) close()"
                    class="block mt-1 mx-3 px-3 py-2 rounded-lg text-sm {{ request()->routeIs('billboard.availability.index') ? 'bg-[#22255b] text-white' : 'hover:bg-neutral-50' }}">
...
{{-- Overlay (mobile only) --}}
<div class="fixed inset-0 bg-black/40 z-40 md:hidden" x-show="isOpen" x-transition.opacity @click="close()" x-cloak>
</div>

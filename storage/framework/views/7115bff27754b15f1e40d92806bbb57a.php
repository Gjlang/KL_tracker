      ...
      <?php $__empty_1 = true; $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <tr class="transition-all duration-200 hover:bg-neutral-50 hover:shadow-sm group">
          <td class="px-3 py-4 whitespace-nowrap tabular-nums ink text-[#1C1E26]">
            <?php echo e(optional($r->date)->format('d/m/Y') ?? '—'); ?>

          </td>
          <td class="px-3 py-4 whitespace-nowrap tabular-nums text-neutral-700">
            <?php echo e(optional($r->expected_finish_date)->format('d/m/Y') ?? '—'); ?>

          </td>
          <td class="px-3 py-4 text-neutral-800 font-medium">
            <div class="truncate w-full" title="<?php echo e($r->company ?? 'N/A'); ?>">
              <?php echo e($r->company ?? '—'); ?>

            </div>
          </td>
          <td class="px-3 py-4 text-neutral-700">
            <div class="truncate w-full" title="<?php echo e($r->client ?? 'N/A'); ?>">
              <?php echo e($r->client ?? '—'); ?>

            </div>
          </td>
          <td class="px-3 py-4 text-neutral-700">
            <div class="truncate w-full" title="<?php echo e($r->product ?? 'N/A'); ?>">
              <?php echo e($r->product ?? '—'); ?>

            </div>
          </td>
          <td class="px-3 py-4 text-neutral-700">
            <div class="truncate w-full" title="<?php echo e($r->servicing ?? 'N/A'); ?>">
              <?php echo e($r->servicing ?? '—'); ?>

            </div>
          </td>
          <td class="px-3 py-4 text-neutral-700">
            <div class="truncate w-full" title="<?php echo e($r->location ?? 'N/A'); ?>">
              <?php echo e($r->location ?? '—'); ?>
          ...
          <td class="px-3 py-4 text-neutral-700">
            <div class="truncate w-full" title="<?php echo e($r->attended_by ?? 'N/A'); ?>">
              <?php echo e($r->attended_by ?? '—'); ?>

            </div>
          </td>
          <td class="px-3 py-4 text-neutral-600 text-xs">
            <div class="truncate w-full" title="<?php echo e($r->reasons ?? 'N/A'); ?>">
              <?php echo e($r->reasons ?? '—'); ?>
          ...
          <td class="px-3 py-4 whitespace-nowrap text-right text-sm">
            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
              <?php if(isset($r->id)): ?>
                <a href="<?php echo e(route('information.booth.edit', $r->id)); ?>"
                ...
                <form method="POST" action="<?php echo e(route('information.booth.destroy', $r->id)); ?>" class="inline">
                  <?php echo csrf_field(); ?>
    ...
    <div class="bg-white border hairline border-neutral-200 rounded-xl p-4 shadow-sm">
      <div class="flex items-start justify-between mb-3">
        <div class="flex-1 min-w-0 mr-3">
          <p class="font-medium text-[#1C1E26] ink truncate"><?php echo e($r->company ?? 'Unknown Company'); ?></p>
          <p class="text-sm text-neutral-600 truncate"><?php echo e($r->client ?? 'No client specified'); ?></p>
        </div>
      ...
      <div class="grid grid-cols-2 gap-3 text-sm mb-3">
        <div>
          ...
          <p class="text-neutral-800 tabular-nums"><?php echo e(optional($r->date)->format('d/m/Y') ?? '—'); ?></p>
        </div>
        <div>
          <p class="text-xs text-neutral-500 uppercase tracking-wide">Expected Finish</p>
          <p class="text-neutral-800 tabular-nums"><?php echo e(optional($r->expected_finish_date)->format('d/m/Y') ?? '—'); ?></p>
        </div>
        <?php if($r->product): ?>
        <div>
          <p class="text-xs text-neutral-500 uppercase tracking-wide">Product</p>
          <p class="text-neutral-700 truncate"><?php echo e($r->product); ?></p>
        </div>
        <?php endif; ?>
        <?php if($r->location): ?>
        <div>
          <p class="text-xs text-neutral-500 uppercase tracking-wide">Location</p>
          <p class="text-neutral-700 truncate"><?php echo e($r->location); ?></p>
        </div>
        <?php endif; ?>
      </div>

      <?php if($r->attended_by || $r->reasons): ?>
      <div class="pt-3 border-t hairline border-neutral-200">
        <?php if($r->attended_by): ?>
          <p class="text-xs text-neutral-500 mb-1">Attended by: <span class="text-neutral-700"><?php echo e($r->attended_by); ?></span></p>
        <?php endif; ?>
        <?php if($r->reasons): ?>
          <p class="text-xs text-neutral-500">Notes: <span class="text-neutral-700"><?php echo e(\Illuminate\Support\Str::limit($r->reasons, 50)); ?></span></p>
        <?php endif; ?>
      </div>
      <?php endif; ?>
...
<?php if(method_exists($rows, 'links') && $rows->hasPages()): ?>
  <div class="px-6 py-4 bg-neutral-50/50 border-t hairline border-neutral-200">
    <div class="flex items-center justify-between">
      <div class="text-sm text-neutral-600">
        Showing <?php echo e($rows->firstItem() ?? 0); ?> to <?php echo e($rows->lastItem() ?? 0); ?> of <?php echo e($rows->total() ?? 0); ?> results
      </div>
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\Users\Gjlang\kl_guide_tracker\resources\views/information_booth/_table.blade.php ENDPATH**/ ?>